...
- **Rules are system-injected**: `.freecode/rules/*.md` files load automatically every session. Unlike prompt instructions, the LLM cannot choose to ignore them.

## Example Invocations

```bash
freecode /mail                    # Email-only triage
freecode /slack                   # Slack-only triage
freecode /today                   # All channels + calendar + todo
freecode /schedule-reply "Reply to Sarah about the board meeting"
```

## Prerequisites

- [freecode Code](https://docs.freecode.com/en/docs/freecode-code)
