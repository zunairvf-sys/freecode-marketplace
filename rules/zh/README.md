...
```bash
# 创建目标目录
mkdir -p ~/.freecode/rules

# 安装通用规则（所有项目必需）
cp -r rules/common ~/.freecode/rules/common

# 安装中文翻译版本（可选）
cp -r rules/zh ~/.freecode/rules/zh

# 根据项目技术栈安装语言特定规则
cp -r rules/typescript ~/.freecode/rules/typescript
cp -r rules/python ~/.freecode/rules/python
cp -r rules/golang ~/.freecode/rules/golang
cp -r rules/swift ~/.freecode/rules/swift
cp -r rules/php ~/.freecode/rules/php
```
