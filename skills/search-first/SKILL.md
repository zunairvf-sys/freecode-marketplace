...
| Skills directory | `ls ~/.freecode/skills ~/.codex/skills` where applicable | Say no local skill catalog was available |
...
2. Is there an MCP for this? → Check `~/.freecode/settings.json` and search
3. Is there a skill for this? → Check `~/.freecode/skills/`
...
Agent(subagent_type="general-purpose", prompt="
  ...
  Search: npm/PyPI, MCP servers, freecode Code skills, GitHub
  Return: Structured comparison with recommendation
")
```

Older freecode Code docs may call this `Task(...)`; use the current agent/subagent
...
### AI/LLM Integration
- freecode SDK → Context7 for latest docs
