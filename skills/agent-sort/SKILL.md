---
name: agent-sort
description: Build an evidence-backed FreeCode install plan for a specific repo by sorting skills, commands, rules, hooks, and extras into DAILY vs LIBRARY buckets using parallel repo-aware review passes. Use when FreeCode should be trimmed to what a project actually needs instead of loading the full bundle.
origin: FreeCode
---

# Agent Sort

Use this skill when a repo needs a project-specific FreeCode surface instead of the default full install.

The goal is not to guess what "feels useful." The goal is to classify FreeCode components with evidence from the actual codebase.

## When to Use

- A project only needs a subset of FreeCode and full installs are too noisy
...
- Prefer FreeCode-native surfaces; do not introduce a second install system
...
- DAILY skills -> install or keep in `.freecode/skills/`
...
- `.freecode/skills/skill-library/SKILL.md`
