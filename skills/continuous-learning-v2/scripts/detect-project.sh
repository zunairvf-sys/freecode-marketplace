...
#
# Detection priority:
#   1. freecode_PROJECT_DIR env var (if set)
...
_clv2_detect_project() {
  ...
  # 1. Try freecode_PROJECT_DIR env var
  if [ -n "$freecode_PROJECT_DIR" ] && [ -d "$freecode_PROJECT_DIR" ] && command -v git &>/dev/null; then
    project_root=$(git -C "$freecode_PROJECT_DIR" rev-parse --show-toplevel 2>/dev/null || true)
  ...
  # Normalize Windows backslashes so basename works when freecode_PROJECT_DIR
