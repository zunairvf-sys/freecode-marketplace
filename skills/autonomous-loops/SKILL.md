...
description: "Patterns and architectures for autonomous freecode Code loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems."
origin: FreeCode
---
...
Patterns, architectures, and reference implementations for running freecode Code autonomously in loops. Covers everything from simple `freecode -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
|---------|-----------|----------|
| [Sequential Pipeline](#1-sequential-pipeline-freecode--p) | Low | Daily dev steps, scripted workflows |
...
| [Continuous freecode PR Loop](#4-continuous-freecode-pr-loop) | Medium | Multi-day iterative projects with CI gates |
...
## 1. Sequential Pipeline (`freecode -p`)

**The simplest loop.** Break daily development into a sequence of non-interactive `freecode -p` calls. Each call is a focused step with a clear prompt.

### Core Insight

> If you can't figure out a loop like this, it means you can't even drive the LLM to fix your code in interactive mode.

The `freecode -p` flag runs freecode Code non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
freecode -p "Read the spec in docs/auth-spec.md. Implement OAuth2 login in src/auth/. Write tests first (TDD). Do NOT create any new documentation files."

# Step 2: De-sloppify (cleanup pass)
freecode -p "Review all files changed by the previous commit. Remove any unnecessary type tests, overly defensive checks, or testing of language features (e.g., testing that TypeScript generics work). Keep real business logic tests. Run the test suite after cleanup."

# Step 3: Verify
freecode -p "Run the full build, lint, type check, and test suite. Fix any failures. Do not add new features."

# Step 4: Commit
freecode -p "Create a conventional commit for all staged changes. Use 'feat: add OAuth2 login flow' as the message."
```

### Key Design Principles

1. **Each step is isolated** — A fresh context window per `freecode -p` call means no context bleed between steps.
...
freecode -p --model opus "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with Sonnet (fast, capable)
freecode -p "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with Opus (thorough)
freecode -p --model opus "Review all changes for security issues, race conditions, and edge cases..."
```

**With environment context:**
```bash
# Pass context via files, not prompt length
echo "Focus areas: auth module, API rate limiting" > .freecode-context.md
freecode -p "Read .freecode-context.md for priorities. Work through them in order."
rm .freecode-context.md
```

**With `--allowedTools` restrictions:**
```bash
# Read-only analysis pass
freecode -p --allowedTools "Read,Grep,Glob" "Audit this codebase for security vulnerabilities..."

# Write-only implementation pass
freecode -p --allowedTools "Read,Write,Edit,Bash" "Implement the fixes from security-audit.md..."
```
...
**FreeCode's built-in persistent loop.** A session-aware REPL that calls `freecode -p` synchronously with full conversation history.
...
1. Loads conversation history from `~/.freecode/claw/{session}.md`
2. Each user message is sent to `freecode -p` with full history as context
...
### Implementation via freecode Code Commands

Create `.freecode/commands/infinite.md`:
...
## 4. Continuous freecode PR Loop

**A production-grade shell script** that runs freecode Code in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).

### Core Loop

```
┌─────────────────────────────────────────────────────┐
│  CONTINUOUS freecode ITERATION                        │
│                                                     │
│  1. Create branch (continuous-freecode/iteration-N)   │
│  2. Run freecode -p with enhanced prompt              │
│  3. (Optional) Reviewer pass — separate freecode -p   │
│  4. Commit changes (freecode generates message)       │
│  5. Push + create PR (gh pr create)                 │
│  6. Wait for CI checks (poll gh pr checks)          │
│  7. CI failure? → Auto-fix pass (freecode -p)         │
...
> **Warning:** Install continuous-freecode from its repository after reviewing the code. Do not pipe external scripts directly to bash.

### Usage

```bash
# Basic: 10 iterations
continuous-freecode --prompt "Add unit tests for all untested functions" --max-runs 10

# Cost-limited
continuous-freecode --prompt "Fix all linter errors" --max-cost 5.00

# Time-boxed
continuous-freecode --prompt "Improve test coverage" --max-duration 8h
...
continuous-freecode --prompt "Add tests" --max-runs 5 --worktree tests-worker &
continuous-freecode --prompt "Refactor code" --max-runs 5 --worktree refactor-worker &
wait
```
...
freecode reads this file at iteration start and updates it at iteration end. This bridges the context gap between independent `freecode -p` invocations.

### CI Failure Recovery

When PR checks fail, Continuous freecode automatically:
1. Fetches the failed run ID via `gh run list`
2. Spawns a new `freecode -p` with CI fix context
3. freecode inspects logs via `gh run view`, fixes code, commits, pushes
4. Re-waits for checks (up to `--ci-retry-max` attempts)

### Completion Signal

freecode can signal "I'm done" by outputting a magic phrase:

```bash
continuous-freecode \
  --prompt "Fix all bugs in the issue tracker" \
  --completion-signal "CONTINUOUS_freecode_PROJECT_COMPLETE" \
...
freecode -p "Implement the feature with full TDD. Be thorough with tests."

# Step 2: De-sloppify (separate context, focused cleanup)
freecode -p "Review all changes in the working tree. Remove:
...
for feature in "${features[@]}"; do
  # Implement
  freecode -p "Implement $feature with TDD."

  # De-sloppify
  freecode -p "Cleanup pass: review changes, remove test/code slop, run tests."

  # Verify
  freecode -p "Run build + lint + tests. Fix any failures."

  # Commit
  freecode -p "Commit with message: feat: add $feature"
done
```
...
| Multi-day project | Yes | Maybe (continuous-freecode) |
...
└─ No → Is there a written spec/RFC?
         ...
         │        └─ No → Continuous freecode (iterative PR loop)
...
2. **Continuous freecode + De-Sloppify** — Add `--review-prompt` with a de-sloppify directive to each iteration.

3. **Any loop + Verification** — Use FreeCode's `/verify` command or `verification-loop` skill as a gate before commits.
...
4. **Ralphinho's tiered approach in simpler loops** — Even in a sequential pipeline, you can route simple tasks to Haiku and complex tasks to Opus:
   ...
   freecode -p --model haiku "Fix the import ordering in src/utils.ts"

   # Complex architectural change
   freecode -p --model opus "Refactor the auth module to use the strategy pattern"
   ```
...
2. **No context bridge between iterations** — Each `freecode -p` call starts fresh. Use `SHARED_TASK_NOTES.md` or filesystem state to bridge context.
...
| Continuous freecode | AnandChowdhary | credit: @AnandChowdhary |
| NanoClaw | FreeCode | `/claw` command in this repo |
| Verification Loop | FreeCode | `skills/verification-loop/` in this repo |
