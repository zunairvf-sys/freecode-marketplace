...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond freecode Code's native Markdown skill loader.

## Installation

This skill ships with Everything freecode Code. No separate installation is needed when FreeCode is installed.

### Full FreeCode install

If you are working from the FreeCode repository checkout, verify the skill is present with:

```bash
test -f skills/blueprint/SKILL.md
```

To update later, review the FreeCode diff before updating:

```bash
cd /path/to/everything-freecode-code
...
If you are vendoring only this skill outside the full FreeCode install, copy the reviewed file from the FreeCode repository into `~/.freecode/skills/blueprint/SKILL.md`. Vendored copies do not have a git remote, so update them by re-copying the file from a reviewed FreeCode commit rather than running `git pull`.

## Requirements

- freecode Code (for `/blueprint` slash command)
