---
name: freecode-devfleet
description: Orchestrate multi-agent coding tasks via freecode DevFleet — plan projects, dispatch parallel agents in isolated worktrees, monitor progress, and read structured reports.
origin: community
---

# freecode DevFleet Multi-Agent Orchestration

## When to Use

Use this skill when you need to dispatch multiple freecode Code agents to work on coding tasks in parallel. Each agent runs in an isolated git worktree with full tooling.

Requires a running freecode DevFleet instance connected via MCP:
```bash
freecode mcp add devfleet --transport http http://localhost:18801/mcp
```
