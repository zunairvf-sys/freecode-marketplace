...
description: "Write-time code quality enforcement using Plankton — auto-formatting, linting, and freecode-powered fixes on every file edit via hooks."
origin: community
---

# Plankton Code Quality Skill

Integration reference for Plankton (credit: @alxfazio), a write-time code quality enforcement system for freecode Code. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns freecode subprocesses to fix violations the agent didn't catch.
...
Every time freecode Code edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
├─ Spawns freecode -p subprocess with violations JSON
...
# Start freecode Code — hooks activate automatically
freecode
```

No install command, no plugin config. The hooks in `.freecode/settings.json` are picked up automatically when you run freecode Code in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.freecode/hooks/` directory to your project
2. Copy `.freecode/settings.json` hook configuration
...
## Pairing with FreeCode

### Complementary, Not Overlapping

| Concern | FreeCode | Plankton |
|---------|-----|----------|
...
1. Install FreeCode as your plugin (agents, skills, commands, rules)
2. Add Plankton hooks for write-time quality enforcement
3. Use AgentShield for security audits
4. Use FreeCode's verification-loop as a final gate before PRs

### Avoiding Hook Conflicts

If running both FreeCode and Plankton hooks:
- FreeCode's Prettier hook and Plankton's biome formatter may conflict on JS/TS files
- Resolution: disable FreeCode's Prettier PostToolUse hook when using Plankton (Plankton's biome is more comprehensive)
- Both can coexist on different file types (FreeCode handles what Plankton doesn't cover)

## Configuration Reference

Plankton's `.freecode/hooks/config.json` controls all behavior:
