---
name: agentic-os
description: Build persistent multi-agent operating systems on freecode Code. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases.
origin: FreeCode
---

# Agentic OS

Treat freecode Code as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside freecode Code
- Setting up persistent freecode Code automation that survives session restarts
...
```
project-root/
├── freecode.md          # Kernel: identity, routing rules, agent registry
├── agents/            # Specialist agent definitions (markdown prompts)
├── .freecode/commands/  # Slash commands: user-facing CLI
...
|---|---|---|
| Kernel (`freecode.md`) | Identity, routing, model policies, agent registry | Git-tracked |
| Agents (`agents/`) | Specialist identities with scoped tools and memory | Git-tracked |
| Commands (`.freecode/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
`freecode.md` is the kernel. It acts as the COO / orchestrator. freecode reads it at session start and uses it to route work.

### Kernel Structure

```markdown
# freecode.md - Agentic OS Kernel
...
Each agent is a standalone markdown file in `agents/`. freecode loads the relevant agent file when routing a task.
...
- Test runner access
- MCP servers as configured in `.freecode/mcp.json`
...
For parallel execution, use freecode Code's background task capability or shell scripts that invoke freecode Code with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `.freecode/commands/`. They define reusable workflows.
...
Place command files in `.freecode/commands/<command-name>.md`. freecode Code auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not freecode Code's built-in cron (which dies when the session ends).
...
[Service]
Type=oneshot
ExecStart=/usr/local/bin/freecode --cwd /path/to/project --command /daily-sync
```
...
Starting fresh every time freecode Code opens.
```
...
```markdown
# BAD - API keys in agent files or freecode.md
...
Keep routing declarative in `freecode.md` markdown tables. It is inspectable, editable, and debuggable.

## Best Practices

- [ ] `freecode.md` is under 200 lines and fits in context window
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not freecode Code's session cron
- [ ] Cost tracking: log API spend per session in `data/logs/<date>-costs.json`
- [ ] One project = one Agentic OS. Do not share a single `freecode.md` across unrelated projects.
