...
description: Evidence-first automation inventory and overlap audit workflow for FreeCode. Use when the user wants to know which jobs, hooks, connectors, MCP servers, or wrappers are live, broken, redundant, or missing before fixing anything.
origin: FreeCode
---
...
- the user wants to know what was ported from another agent system and what still needs to be rebuilt inside FreeCode
...
The value is in collapsing noisy automation into one canonical FreeCode lane, not in preserving every historical path.
