 ...
 * - Goal mismatch detection vs freecode.md
...
function extractfreecodeMdGoal(projectPath) {
  const p = resolve(projectPath, 'freecode.md');
...
function main() {
  ...
  if (entry?.contextDir) {
    ...
    if (context) {
      ...
      const freecodeMdGoal = extractfreecodeMdGoal(cwd);
      if (freecodeMdGoal && context.goal &&
          freecodeMdGoal.toLowerCase().trim() !== context.goal.toLowerCase().trim()) {
        summaryLines.push(`WARNING Goal mismatch — ck: "${context.goal.slice(0, 40)}" · freecode.md: "${freecodeMdGoal.slice(0, 40)}"`);
      ...
      // Instruct freecode to display compact briefing at session start
      parts.push([
        `---`,
        `## ck: SESSION START`,
        ``,
