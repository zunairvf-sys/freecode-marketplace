 ...
 * init.mjs — auto-detect project info and output JSON for freecode to confirm
 *
 * Usage: node init.mjs
...
// ── freecode.md ─────────────────────────────────────────────────────────────────
const freecodeMd = readFile('freecode.md');
if (freecodeMd) {
  const goal = extractSection(freecodeMd, 'Current Goal');
  if (goal && !output.goal) output.goal = goal.split('\n')[0].trim();

  const doNot = extractSection(freecodeMd, 'Do Not Do');
  if (doNot) {
  ...
  const stack = extractSection(freecodeMd, 'Tech Stack');
  if (stack && output.stack.length === 0) {
    output.stack = stack.split(/[,\n]/).map(s => s.replace(/^[-*]\s+/, '').trim()).filter(Boolean);
  }

  // Description from first section or "What This Is"
  const whatItIs = extractSection(freecodeMd, 'What This Is') || extractSection(freecodeMd, 'About');
