...
```bash
python3 "${freecode_PLUGIN_ROOT}/skills/continuous-learning-v2/scripts/instinct-cli.py" promote [instinct-id] [--force] [--dry-run]
```

Or if `freecode_PLUGIN_ROOT` is not set (manual installation):

```bash
python3 ~/.freecode/skills/continuous-learning-v2/scripts/instinct-cli.py promote [instinct-id] [--force] [--dry-run]
```
...
4. Write promoted instincts to `~/.freecode/homunculus/instincts/personal/` with `scope: global`
