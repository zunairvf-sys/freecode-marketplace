...
- **Code Sovereignty**: External models have **zero filesystem write access**, all modifications by freecode
- **Stop-Loss Mechanism**: Do not proceed to next phase until current phase output is validated
- **Planning Only**: This command allows reading context and writing to `.freecode/plan/*` plan files, but **NEVER modify production code**
...
```
Bash({
  command: "~/.freecode/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Analysis | `~/.freecode/.ccg/prompts/codex/analyzer.md` | `~/.freecode/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.freecode/.ccg/prompts/codex/architect.md` | `~/.freecode/.ccg/prompts/gemini/architect.md` |
...
**If ace-tool MCP is NOT available**, use freecode Code built-in tools as fallback:
...
1. **Codex Backend Analysis**:
   - ROLE_FILE: `~/.freecode/.ccg/prompts/codex/analyzer.md`
...
2. **Gemini Frontend Analysis**:
   - ROLE_FILE: `~/.freecode/.ccg/prompts/gemini/analyzer.md`
...
To reduce risk of omissions in freecode's synthesized plan, can parallel have both models output "plan drafts" (still **NOT allowed** to modify files):

1. **Codex Plan Draft** (Backend authority):
   - ROLE_FILE: `~/.freecode/.ccg/prompts/codex/architect.md`
...
2. **Gemini Plan Draft** (Frontend authority):
   - ROLE_FILE: `~/.freecode/.ccg/prompts/gemini/architect.md`
...
#### 2.4 Generate Implementation Plan (freecode Final Version)
...
2. Save plan to `.freecode/plan/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

---
**Plan generated and saved to `.freecode/plan/actual-feature-name.md`**
...
```
/ccg:execute .freecode/plan/actual-feature-name.md
```
---
...
- **First planning**: `.freecode/plan/<feature-name>.md`
- **Iteration versions**: `.freecode/plan/<feature-name>-v2.md`, `.freecode/plan/<feature-name>-v3.md`...
...
2. Update `.freecode/plan/<feature-name>.md` file
...
```bash
/ccg:execute .freecode/plan/<feature-name>.md
```
