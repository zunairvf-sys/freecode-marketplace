---
description: Execute a multi-model implementation plan while preserving freecode as the only filesystem writer.
---

# Execute - Multi-Model Collaborative Execution

Multi-model collaborative execution - Get prototype from plan → freecode refactors and implements → Multi-model audit and delivery.
...
Bash({
  command: "~/.freecode/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Implementation | `~/.freecode/.ccg/prompts/codex/architect.md` | `~/.freecode/.ccg/prompts/gemini/frontend.md` |
| Review | `~/.freecode/.ccg/prompts/codex/reviewer.md` | `~/.freecode/.ccg/prompts/gemini/reviewer.md` |
...
1. **Identify Input Type**:
   - Plan file path (e.g., `.freecode/plan/xxx.md`)
...
1. Call Gemini (use `~/.freecode/.ccg/prompts/gemini/frontend.md`)
...
1. Call Codex (use `~/.freecode/.ccg/prompts/codex/architect.md`)
2. Input: Plan content + retrieved context + target files
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
**freecode as Code Sovereign executes the following steps**:
...
1. **Codex Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.freecode/.ccg/prompts/codex/reviewer.md`
...
2. **Gemini Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.freecode/.ccg/prompts/gemini/reviewer.md`
   - Input: Changed Diff + target files
...
1. **Code Sovereignty** – All file modifications by freecode, external models have zero write access
