...
Hooks are event-driven automations that fire before or after freecode Code tool executions. They enforce code quality, catch mistakes early, and automate repetitive checks.

## How Hooks Work

```
User request → freecode picks a tool → PreToolUse hook runs → Tool executes → PostToolUse hook runs
```
...
- **Stop** hooks run after each freecode response.
...
For freecode Code manual installs, do not paste the raw repo `hooks.json` into `~/.freecode/settings.json` or copy it directly into `~/.freecode/hooks/hooks.json`. The checked-in file is plugin/repo-oriented and is meant to be installed through the FreeCode installer or loaded as a plugin.

Use the installer instead so hook commands are rewritten against your actual freecode root:

```bash
bash ./install.sh --target freecode --modules hooks-runtime
```

```powershell
pwsh -File .\install.ps1 --target freecode --modules hooks-runtime
```

That installs resolved hooks to `~/.freecode/hooks/hooks.json`. On Windows, the freecode config root is `%USERPROFILE%\\.freecode`.
...
| **Doc file warning** | `Write` | Warns about non-standard `.md`/`.txt` files (allows README, freecode, CONTRIBUTING, CHANGELOG, LICENSE, SKILL, docs/, skills/); cross-platform path handling | 0 (warns) |
...
Remove or comment out the hook entry in `hooks.json`. If installed as a plugin, override in your `~/.freecode/settings.json`:
...
process.stdin.on('end', () => {
  ...
  console.error('[Hook] Warning message shown to freecode');
...
This marketplace does not yet bundle a plugin-level Node.js hook runtime (see `hooks/NOTE.md`). Skills that ship their own self-contained hook scripts — e.g. `skills/continuous-learning-v2/hooks/observe.sh` — remain usable via manual `settings.json` registration.
...
- [NOTE.md](./NOTE.md) — status of the hook script runtime in this marketplace
