...
function getTokenPath() {
  const raw = process.env.ZOOM_TOKEN_FILE;
  if (raw && raw !== "" && !raw.startsWith("${")) {
    if (path.isAbsolute(raw)) return raw;
    if (raw.startsWith("~")) return path.join(os.homedir(), raw.slice(1));
    return raw;
  }
  return path.join(os.homedir(), ".freecode", ".zoom-token.json");
}
...
defTool(
  "auth_zoom",
  "Get the OAuth authorization URL for Zoom. Open this URL in your browser and authorize. The redirect is captured automatically. After authorizing, call zoom_auth_status to confirm. If capture fails, copy the 'code' param and pass it to auth_zoom_exchange_code.",
  {},
  async () => {
    startCallbackServer(REDIRECT_URI, (code) => zoom.exchangeCode(code));
    if (await zoom.ensureAuthenticated()) {
      return { content: [{ type: "text", text: `Already authenticated with Zoom. Call zoom_auth_status to verify.\n\nFresh auth URL (if needed):\n${zoom.getAuthUrl()}` }] };
    }
    return { content: [{ type: "text", text: `Open this URL in your browser to authorize Zoom access:\n\n${zoom.getAuthUrl()}\n\nAfter authorizing, call zoom_auth_status to confirm the token was saved.` }] };
  }
);

defTool(
  "zoom_auth_status",
  "Check whether Zoom authentication is active and the stored token is valid. Call this after auth_zoom to confirm before using other tools.",
  {},
  async () => {
    const t = await getToken();
    if (!t) return { content: [{ type: "text", text: `Not authenticated.\nToken file: ${getTokenPath()}\nCall auth_zoom to start the OAuth flow.` }] };
    ...
    return { content: [{ type: "text", text: `Authenticated with Zoom.\nToken file: ${getTokenPath()}\nAccess token: present (${expiryStr})\nRefresh token: ${t.refresh_token ? "present" : "missing"}\n\nYou can use Zoom tools now.` }] };
  }
);
