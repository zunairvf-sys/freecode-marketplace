...
# Feeds FreeCode's output back as input to continue the loop
...
# fires in every FreeCode Code session in that project. If another session
...
if [[ ! -f "$TRANSCRIPT_PATH" ]]; then
  ...
  echo "   This is unusual and may indicate a FreeCode Code internal issue." >&2
...
#
# FreeCode Code writes each content block (text/tool_use/thinking) as its own
...
# The "reason" field contains the prompt that will be sent back to FreeCode
jq -n \
