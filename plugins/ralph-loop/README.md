...
Implementation of the Ralph Wiggum technique for iterative, self-referential AI development loops in FreeCode Code.
...
This plugin implements Ralph using a **Stop hook** that intercepts FreeCode's exit attempts:

```bash
# You run ONCE:
/ralph-loop "Your task description" --completion-promise "DONE"

# Then FreeCode Code automatically:
# 1. Works on the task
# 2. Tries to exit
...
- FreeCode's previous work persists in files
- Each iteration sees modified files and git history
- FreeCode autonomously improves by reading its own past work in files
...
```json
"command": "\"C:/Program Files/Git/bin/bash.exe\" ${FREECODE_PLUGIN_ROOT}/hooks/stop-hook.sh"
```

**Location**: `~/.freecode/plugins/cache/freecode-plugins-official/ralph-wiggum/<hash>/hooks/hooks.json`
...
Run `/help` in FreeCode Code for detailed command reference and examples.
