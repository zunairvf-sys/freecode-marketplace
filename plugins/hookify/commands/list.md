...
1. Use Glob tool to find all hookify rule files:
   ```
   pattern: ".freecode/hookify.*.local.md"
   ```
...
**Status**: ✅ Active
**File**: .freecode/hookify.dangerous-rm.local.md
```
...
2. Or manually create `.freecode/hookify.my-rule.local.md` files
...
Check `${FREECODE_PLUGIN_ROOT}/examples/` for example rule files.
```
