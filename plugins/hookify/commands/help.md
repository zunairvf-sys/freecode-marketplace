...
- **Stop**: When FreeCode wants to stop working
- **UserPromptSubmit**: When user submits a prompt

These hooks read configuration files from `.freecode/hookify.*.local.md` and check if any rules match the current operation.

### 2. Configuration Files

Users create rules in `.freecode/hookify.{rule-name}.local.md` files:
...
The message body is what FreeCode sees when the rule triggers.
...
Create `.freecode/hookify.my-rule.local.md` with the format above.
...
**Rule Files**: Keep rules in `.freecode/hookify.*.local.md` - they should be git-ignored (add to .gitignore if needed).
...
**Hook not triggering:**
- Check rule file is in `.freecode/` directory
...
2. Try to trigger it:
   - Ask FreeCode to run `rm -rf /tmp/test`
   - You should see the warning

4. Refine the rule by editing `.freecode/hookify.warn-rm.local.md`

5. Create more rules as you encounter unwanted behaviors

For more examples, check the `${FREECODE_PLUGIN_ROOT}/examples/` directory.
