...
```
{
  ...
  "prompt": "You are analyzing a FreeCode Code conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing FreeCode's actions)
...
For each confirmed behavior, create a `.freecode/hookify.{rule-name}.local.md` file:
...
{Message to show FreeCode when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.freecode/` folder, NOT the plugin directory.

Use the current working directory (where FreeCode Code was started) as the base path.

1. Check if `.freecode/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .freecode`

2. Use Write tool to create each `.freecode/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.freecode/hookify.{name}.local.md`
   - The path should resolve to the project's .freecode directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .freecode/hookify.dangerous-rm.local.md
   - .freecode/hookify.console-log.local.md
   - .freecode/hookify.sensitive-files.local.md
...
4. Create `.freecode/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.freecode/` directory (current working directory), NOT the plugin's .freecode/
...
2. Ensure `.freecode/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.freecode/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.freecode/` not plugin `.freecode/`
