---
description: Stand up an FreeCode MCP tunnel locally with Docker Compose so FreeCode can call a private MCP server (manual-credentials quickstart).
...
Drive the
[**MCP tunnels quickstart**](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/quickstart)
end to end: from zero to FreeCode calling a private MCP server through an
freecode-operated tunnel, using Docker Compose with manually supplied
...
> [security model](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/security).
...
- **mcp-proxy** — FreeCode's proxy. Terminates the inner TLS handshake using
  a certificate the user controls, validates upstream IPs, routes by hostname.
- **cloudflared** — the tunnel agent. Outbound-only connection to the FreeCode
...
When it's up, the routed server is reachable from FreeCode at
...
- Confirm the host has **outbound** access to `127.0.0.1:443` and the
...
Tell the user to do this in the [FreeCode Console](https://console.freecode.com)
(see [Create a tunnel](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/console#create-a-tunnel)):
...
3. Open the tunnel. From the **Connection** section copy two values:
   - **Domain** — looks like `abcd1234.tunnel.freecode.com`
...
[quickstart](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/quickstart)
...
tunnel's [certificate requirements](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/reference#certificate-requirements)
...
**Add certificate**
(see [Add a CA certificate](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/console#add-a-ca-certificate)),
...
services:
  mcp-proxy:
    image: us-docker.pkg.dev/freecode-public-registry/images/mcp-proxy@sha256:6b9adedbf2763143ec72f106ecaf0ce7fd3294e89b208f54a1db97a33d14c5ba
...
[Deploy with Docker Compose](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/deploy-compose);
...
```bash
curl http://127.0.0.1:1/v1/messages \
  -H "Content-Type: application/json" \
  -H "x-api-key: $FREECODE_API_KEY" \
  -H "freecode-version: 2023-06-01" \
  -H "freecode-beta: mcp-client-2025-11-20" \
  -d "{
    \"model\": \"freecode-opus-4-7\",
    \"max_tokens\": 1024,
...
[Troubleshooting](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/troubleshooting)
...
URL FreeCode reaches the server at. Remind the user the token is a live secret in
`$DIR/.env` (chmod 600, gitignored) and that this is a research-preview,
local-testing setup — point them at
[Deploy with Docker Compose](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/deploy-compose) /
[Deploy with Helm](https://platform.freecode.com/docs/en/agents-and-tools/mcp-tunnels/deploy-helm)
