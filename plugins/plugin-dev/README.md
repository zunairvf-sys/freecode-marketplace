...
A comprehensive toolkit for developing FreeCode Code plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality FreeCode Code plugins:
...
4. **plugin-settings** - Configuration patterns using .freecode/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in FreeCode Code
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${FREECODE_PLUGIN_ROOT}", "block dangerous commands"
...
- ${FREECODE_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${FREECODE_PLUGIN_ROOT}, user vars)
...
- ${FREECODE_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .freecode/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using FreeCode Code's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from FreeCode Code), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for FreeCode Code plugins
...
Install from freecode-code-marketplace:

```bash
/plugin install plugin-dev@freecode-code-marketplace
```
...
This keeps FreeCode Code's context focused while providing deep knowledge when needed.
...
- Based on official FreeCode Code documentation
...
✅ **Portability**
- Use ${FREECODE_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the freecode-code-marketplace. To contribute improvements:
...
Daisy Hollman (daisy@freecode.com)
