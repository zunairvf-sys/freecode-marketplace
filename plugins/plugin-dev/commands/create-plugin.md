...
Guide the user through creating a complete, high-quality FreeCode Code plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.freecode-plugin
...
6. Create .gitignore if needed (for .freecode/\*.local.md, etc.)
...
2. For each skill:
   ...
   - For user-invoked skills (slash commands): include `description`, `argument-hint`, and `allowed-tools` frontmatter; write instructions FOR FreeCode (not TO user)
...
2. For each command:
   ...
   - Write instructions FOR FreeCode (not TO user)
...
2. Create .mcp.json configuration with:
   ...
   - Command and args (with ${FREECODE_PLUGIN_ROOT})
...
3. Create example .freecode/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.freecode/*.local.md`
...
5. **Test hook configuration** (if plugin has hooks):
   ...
   - Verify ${FREECODE_PLUGIN_ROOT} usage
...
**Goal**: Test that plugin works correctly in FreeCode Code
...
1. **Installation instructions**:
   - Show user how to test locally:
     ...
     ```
   - Or copy to `.freecode-plugin/` for project testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `freecode --debug` to see hook execution
...
- **Apply best practices**:
  ...
  - Skill instructions written FOR FreeCode (not TO user)
  - Strong trigger phrases
  - ${FREECODE_PLUGIN_ROOT} for portability
