...
Detailed analysis of how production plugins use the `.freecode/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".freecode/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.freecode/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".freecode/multi-agent-swarm.local.md" > temp.md
mv temp.md ".freecode/multi-agent-swarm.local.md"
```
...
Document any changes needed in FREECODE.md.
```
...
**Purpose:** Prevent session exit and loop FreeCode's output back as input
...
RALPH_STATE_FILE=".freecode/ralph-loop.local.md"
...
# Create state file
cat > ".freecode/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .freecode/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.freecode/multi-agent-swarm.local.md` | `.freecode/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.freecode/my-plugin.local.md"

# GOOD
FILE=".freecode/my-plugin.local.md"
```
...
The `.freecode/plugin-name.local.md` pattern provides:
