...
description: This skill should be used when the user asks to "create an agent", "add an agent", "write a subagent", "agent frontmatter", "when to use description", "agent examples", "agent tools", "agent colors", "autonomous agent", or needs guidance on agent structure, system prompts, triggering conditions, or agent development best practices for FreeCode Code plugins.
version: 0.1.0
---

# Agent Development for FreeCode Code Plugins
...
Defines when FreeCode should trigger this agent. **This is the most critical field** — it is loaded into context whenever the agent is registered, so the harness can decide when to dispatch.
...
- `sonnet` - FreeCode Sonnet (balanced)
- `opus` - FreeCode Opus (most capable, expensive)
- `haiku` - FreeCode Haiku (fast, cheap)
...
Use this prompt pattern (extracted from FreeCode Code):
...
3. Check FreeCode loads the agent
...
- **`references/agent-creation-system-prompt.md`** - The exact prompt from FreeCode Code
