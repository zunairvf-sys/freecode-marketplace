...
This reference provides common, proven patterns for implementing FreeCode Code hooks. Use these patterns as starting points for typical hook use cases.
...
```bash
#!/bin/bash
cd "$FREECODE_PROJECT_DIR" || exit 1
...
if [ -f "package.json" ]; then
  ...
  echo "export PROJECT_TYPE=rust" >> "$FREECODE_ENV_FILE"
fi
```
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${FREECODE_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${FREECODE_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$FREECODE_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart FreeCode Code after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$FREECODE_PROJECT_DIR/.freecode/my-plugin.local.json"
...
**Configuration file (.freecode/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
