  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${FREECODE_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".freecode-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .freecode-hooks-config.json)
...
**Example .freecode-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.freecode/audit.log
...
export FREECODE_PROJECT_DIR="/tmp/test-project"
export FREECODE_PLUGIN_ROOT="$(pwd)"
mkdir -p "$FREECODE_PROJECT_DIR"
