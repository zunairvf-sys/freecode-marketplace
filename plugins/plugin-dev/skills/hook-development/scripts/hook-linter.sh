...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$FREECODE_PROJECT_DIR or \$FREECODE_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses FREECODE_PLUGIN_ROOT
  if ! grep -q "FREECODE_PLUGIN_ROOT\|FREECODE_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$FREECODE_PLUGIN_ROOT for plugin-relative paths"
  fi
