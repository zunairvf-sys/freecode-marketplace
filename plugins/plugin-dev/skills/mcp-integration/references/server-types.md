...
Complete reference for all MCP server types supported in FreeCode Code plugins.
...
{
  "my-server": {
    "command": "${FREECODE_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: FreeCode Code spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire FreeCode Code session
4. **Shutdown**: Process terminated when FreeCode Code exits
...
{
  "custom": {
    "command": "${FREECODE_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${FREECODE_PLUGIN_ROOT}**
...
FreeCode Code handles OAuth flow:
