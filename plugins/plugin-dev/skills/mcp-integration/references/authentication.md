...
Complete guide to authentication methods for MCP servers in FreeCode Code plugins.
...
FreeCode Code automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. FreeCode Code detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by FreeCode Code
...
No additional auth configuration needed! FreeCode Code handles everything.
...
Tokens are stored securely by FreeCode Code:
...
```json
{
  "api": {
    ...
    "headersHelper": "${FREECODE_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${FREECODE_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${FREECODE_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
