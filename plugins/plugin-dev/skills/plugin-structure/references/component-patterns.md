...
When FreeCode Code starts:

1. **Scan enabled plugins**: Read `.freecode-plugin/plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to FreeCode Code
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during FreeCode Code initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → FreeCode Code looks up → Executes
**Agents**: Task arrives → FreeCode Code evaluates capabilities → Selects agent
**Skills**: Task context matches description → FreeCode Code loads skill
**Hooks**: Event occurs → FreeCode Code calls matching hooks
...
**Note**: FreeCode Code doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, FreeCode Code doesn't support file references.
...
```bash
#!/bin/bash
source "${FREECODE_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
