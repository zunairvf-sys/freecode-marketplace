...
**Required path**: `.freecode-plugin/plugin.json`

The manifest MUST be in the `.freecode-plugin/` directory at the plugin root. FreeCode Code will not recognize plugins without this file in the correct location.
...
- Plugin identification in FreeCode Code
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${FREECODE_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
When FreeCode Code loads components:
...
FreeCode Code validates the manifest on plugin load:
