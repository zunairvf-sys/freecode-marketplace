...
if [ ! -f ".freecode/command-initialized" ]; then
...
Checking for user config: .freecode/plugin-name.local.md

if [ -f ".freecode/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .freecode/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .freecode/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .freecode/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .freecode/plugin-name.local.md to customize"
fi
...
Or set in .freecode/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".freecode/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .freecode/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .freecode/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .freecode/operation-completed.flag
```
