...
This reference covers features and patterns specific to commands bundled in FreeCode Code plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [FREECODE_PLUGIN_ROOT Environment Variable](#freecode_plugin_root-environment-variable)
...
FreeCode Code automatically discovers commands in plugins using the following locations:
...
## FREECODE_PLUGIN_ROOT Environment Variable

### Purpose

`${FREECODE_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${FREECODE_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${FREECODE_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${FREECODE_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${FREECODE_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${FREECODE_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${FREECODE_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${FREECODE_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${FREECODE_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${FREECODE_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${FREECODE_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${FREECODE_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${FREECODE_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${FREECODE_PLUGIN_ROOT}`
...
Load configuration: @${FREECODE_PLUGIN_ROOT}/deploy-config.json
...
Template: @${FREECODE_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${FREECODE_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${FREECODE_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${FREECODE_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${FREECODE_PLUGIN_ROOT}/cache && date > ${FREECODE_PLUGIN_ROOT}/cache/last-run.txt`
...
- FreeCode will automatically use Task tool to launch agent
...
- Mention skill by name to hint FreeCode should invoke it
...
- Config exists: !`test -f ${FREECODE_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${FREECODE_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${FREECODE_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${FREECODE_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${FREECODE_PLUGIN_ROOT} for all plugin-internal paths**
