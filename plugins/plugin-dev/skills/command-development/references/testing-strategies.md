...
head -n 20 .freecode/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .freecode/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .freecode/commands/*.md

# Check file is in correct location
test -f .freecode/commands/my-command.md && echo "Found" || echo "Missing"
```
...
# 5. Check debug logs
tail -f ~/.freecode/debug-logs/latest
...
cat > .freecode/commands/test-bash.md << 'EOF'
---
...
cat > .freecode/commands/test-forbidden.md << 'EOF'
---
...
# Command: .freecode/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.freecode/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .freecode/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .freecode/commands/; then
...
```bash
# Monitor FreeCode Code during command execution
# In terminal 1:
freecode --debug

# In terminal 2:
watch -n 1 'ps aux | grep freecode'
...
```bash
# Check file location
ls -la .freecode/commands/my-command.md

# Check permissions
chmod 644 .freecode/commands/my-command.md

# Check syntax
head -n 20 .freecode/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .freecode/commands/my-command.md
grep '\$ARGUMENTS' .freecode/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .freecode/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .freecode/commands/my-command.md

# Verify command syntax
grep '!\`' .freecode/commands/my-command.md
