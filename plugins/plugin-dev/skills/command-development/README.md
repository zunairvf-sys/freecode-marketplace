...
Comprehensive guidance on creating FreeCode Code slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${FREECODE_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${FREECODE_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
FreeCode Code activates this skill when users:
...
FreeCode loads references and examples as needed based on task.
...
- **Project**: `.freecode/commands/` (shared with team)
- **Personal**: `~/.freecode/commands/` (your commands)
...
- ✓ Plugin command patterns (${FREECODE_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current FreeCode Code
