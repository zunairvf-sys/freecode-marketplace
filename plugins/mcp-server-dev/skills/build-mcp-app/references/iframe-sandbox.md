...
MCP-app widgets run inside a sandboxed `<iframe>` in the host (FreeCode Desktop,
freecode.local). The sandbox and CSP attributes lock down what the widget can do.
...
The iframe has its own console. In FreeCode Desktop, open DevTools (View → Toggle
