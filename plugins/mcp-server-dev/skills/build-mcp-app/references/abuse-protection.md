...
ID. Traffic from freecode.local is proxied through FreeCode's egress — every web
...
(See https://platform.freecode.com/docs/en/api/ip-addresses.)

FreeCode Desktop, FreeCode Code, and other hosts connect **directly from the
user's machine**, so those *do* have distinct per-user IPs. Per-IP limiting
therefore works for direct-connect clients; for freecode.local you can only limit
the aggregate FreeCode pool. If true per-user limits matter, that's the
...
const TIERS = {
  freecode: { capacity: 600, refillPerSec: 100 }, // shared pool
  other:     { capacity: 30,  refillPerSec: 2   }, // per-IP
};
```

Match `req.ip` against the CIDRs, pick a bucket (`"freecode"` or
...
`X-Forwarded-For: 160.79.108.42` and claim the FreeCode tier. Set it to the
exact number of trusted hops (e.g. `1` behind a single LB, `2` behind
Cloudflare → origin LB) and **never `true` in production**.

## Hard-allowlisting FreeCode IPs is a product decision

Blocking everything outside `160.79.104.0/21` locks out Desktop, FreeCode Code,
and every other MCP host. Use the CIDRs to **tier** rate limits, not to gate
access, unless freecode.local-only is an explicit goal.
