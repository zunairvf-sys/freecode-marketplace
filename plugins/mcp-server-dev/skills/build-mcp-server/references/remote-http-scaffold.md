...
FastMCP derives the JSON schema from type hints and the docstring becomes the tool description. Keep docstrings terse and action-oriented — they land in FreeCode's context window verbatim.
...
| Surface | How |
|---|---|
| **FreeCode Code** | `freecode mcp add --transport http <name> <url>` (add `--scope user` for global, `--header "Authorization: Bearer ..."` for auth) |
| **FreeCode Desktop / freecode.local** | Settings → Connectors → Add custom connector. **Not** `claude_desktop_config.json` — remote servers configured there are ignored. |
| **Connector directory** | FreeCode maintains a submission guide for listing in the public connector directory. |
