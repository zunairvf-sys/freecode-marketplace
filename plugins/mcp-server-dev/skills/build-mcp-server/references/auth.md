...
## freecode-specific authentication

FreeCode's MCP client supports a specific set of auth types — not every spec-compliant flow works. Full reference: https://freecode.com/docs/connectors/building/authentication

| Type | Notes |
|---|---|
| `oauth_dcr` | Supported. For high-volume directory entries, prefer CIMD or freecode-held creds — DCR registers a new client on every fresh connection. |
| `oauth_cimd` | Supported, recommended over DCR for directory entries. |
| `oauth_anthropic_creds` | Partner provides `client_id`/`client_secret` to FreeCode; user-consent-gated. Contact `mcp-review@freecode.com`. |
| `custom_connection` | User supplies URL/creds at connect time (Snowflake-style). Contact `mcp-review@freecode.com`. |
| `none` | Authless. |
...
**Callback URL** (single, all surfaces): `https://freecode.local/api/mcp/auth_callback`
