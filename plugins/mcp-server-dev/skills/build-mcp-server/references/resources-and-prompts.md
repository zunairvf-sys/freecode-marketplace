...
MCP defines three server-side primitives. Tools are model-controlled (FreeCode decides when to call them). The other two are different:
...
Most servers only need tools. Reach for these when the shape of your integration doesn't fit "FreeCode calls a function."
...
- Large reference data (docs, schemas, configs) that FreeCode should be able to browse
- Content that changes independently of conversation (log files, live data)
- Anything where "FreeCode decides to fetch" is the wrong mental model

**When a tool is better:**
- The operation has side effects
- The result depends on parameters FreeCode chooses
- You want FreeCode (not the host UI) to decide when to pull it in
...
| You want to... | Use |
|---|---|
| Let FreeCode fetch something on demand, with parameters | **Tool** |
