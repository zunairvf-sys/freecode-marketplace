...
One line of config, lands directly in FreeCode's system prompt. Use it for tool-use hints that don't fit in individual tool descriptions.
...
This is the highest-leverage one-liner in the spec. If FreeCode keeps misusing your tools, put the fix here.
...
**Requires client support** — check `clientCapabilities.sampling` first. Model preference hints are substring-matched (`"freecode-3-5"` matches any FreeCode 3.5 variant).
...
| Elicitation | — | `elicitation: {}` | return text, ask FreeCode to relay |
