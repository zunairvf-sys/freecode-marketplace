...
def state_dir():
    ...
    Resolution precedence (highest first):
      ...
      2. FREECODE_CONFIG_DIR/security  — CC's config-dir env var (#1868)
      3. ~/.freecode/security          — default fallback

    Empty-string env vars are treated as not-set so a misconfigured shell
    (`FREECODE_CONFIG_DIR=` with no value) doesn't silently write to
    ...
    cc_config = os.environ.get("FREECODE_CONFIG_DIR")
    if cc_config:
        return os.path.expanduser(os.path.join(cc_config, "security"))
    return os.path.expanduser("~/.freecode/security")


# Debug log file. Lives under the plugin state dir (default ~/.freecode/security/)
...
# (plugin-specific override) or FREECODE_CONFIG_DIR (CC-wide config dir, #1868).
...
def debug_log(message):
    ...
    try:
        ...
        # creates ~/.freecode/security/ if it isn't already there. 0700 so other
...
PROVENANCE_TAG = "[from security-guidance@freecode-code-plugins plugin]"
PROVENANCE_BANNER = (
    "[from security-guidance@freecode-code-plugins plugin — automated "
...
def _read_plugin_version_int():
    ...
    try:
        with open(os.path.join(os.path.dirname(__file__), "..", ".freecode-plugin", "plugin.json")) as f:
...
# module-global is naturally per-invocation. _call_freecode_dual_or and
...
_USAGE = {
    ...
    # _usage_metrics). Without this, API failures from `_call_freecode` left
...
_PRICE_PER_MTOK = {
    "freecode-haiku-4-5": (1.0, 5.0),
    "freecode-sonnet-4-6": (3.0, 15.0),
    "freecode-opus-4-6": (15.0, 75.0),
    "freecode-opus-4-7": (5.0, 25.0),
}
_PRICE_DEFAULT = (3.0, 15.0)


def _record_usage(usage, model, cost_usd=None):
    """Accumulate one API response's token usage. `usage` is the FreeCode
    ...
    (longest-prefix match so `freecode-sonnet-4-6-20251015` → sonnet row)."""
