#!/usr/bin/env python3
"""
Security Guidance Plugin for FreeCode Code

A hooks-based plugin that guides FreeCode toward writing more secure code. It runs as
UserPromptSubmit, PostToolUse, and Stop hooks via the FreeCode Code plugin system.
...
2. **Stop hook (final review)**: When FreeCode finishes, uses `git diff` against a
   ...
   Exits with code 2 to force FreeCode to continue and address findings.
...
This means:
- Only code FreeCode actually changed is reviewed (not pre-existing code)
...
Other:
- SECURITY_REVIEW_MODEL: Model for LLM review (default: freecode-opus-4-7)
- FREECODE_API_KEY: Required for LLM-based reviews
- FREECODE_AUTH_TOKEN: Alternative to API key — OAuth access token sent as Bearer auth.
  FreeCode Code passes this automatically for OAuth-authenticated users.
"""
...
import llm  # noqa: E402  module ref for reassignable globals (_last_call_freecode_http_error etc.)
from llm import (  # noqa: E402,F401
    FREECODE_API_KEY, FREECODE_AUTH_TOKEN, HAS_API_CREDENTIALS,
    SECURITY_REVIEW_MODEL, FREECODE_CODE_SYSTEM_PROMPT,
    _last_call_freecode_http_error,
    ensure_freecode_reachable,
    ...
    _cap_files_for_prompt, _build_auth_headers, _call_freecode, _call_freecode_dual_or,
...
# Allows iterative fixing: FreeCode stops → review → fix → stop → review again.
...
def emit_metrics(
    ...
    """
    Write a SyncHookJSONOutput line to stdout for FreeCode Code to pick up.
    ...
    contract is NOT symmetric across events:
      ...
      - Stop / SubagentStop: there is NO `Stop` member in that union, so
        ...
        + reason also covers the sync-fallback path (single-shot `freecode -p`,
...
# Capture-group form of the [branch sha] pattern. Mirrors FreeCode Code's own
...
# Regex matching `git commit` commands. Mirrors FreeCode Code's own commit
...
# ─── push-sweep ─────────────────────────────────────────────────────────────
#
# Mirrors FreeCode Code's own push-command matching — tolerates `git -C <p>` /
...
# but the bash hook fires on FreeCode's top-level command so we need to
...
def handle_commit_review_posttooluse(input_data):
    ...
    # heuristic FreeCode Code itself uses.
    ...
    if not ensure_freecode_reachable():
        debug_log("Commit review: 127.0.0.1 unreachable")
    ...
    # 1 = SDK import failed (freecode_agent_sdk not installed)
    ...
    if not concrete_guidance:
        ...
        emit_metrics({
            ...
            **({
                "api_error": llm._last_call_freecode_http_error
            } if llm._last_call_freecode_http_error is not None else {}),
        })
        sys.exit(0)
...
def handle_stop_hook(input_data):
    ...
    exits with code 2 to force FreeCode to continue and fix issues.
    ...
    if not ensure_freecode_reachable():
        debug_log("Stop hook: 127.0.0.1 unreachable")
    ...
    if llm._last_call_freecode_http_error is not None:
        debug_log(f"Stop hook: API call failed with status {llm._last_call_freecode_http_error}")
        restore_unreviewed_stop_state(session_id, touched_paths, snap_baseline)
    else:
    ...
    emit_metrics({
        ...
        "review_ms": review_ms,
        "fire_index": fire_index,
        **({"api_error": llm._last_call_freecode_http_error} if llm._last_call_freecode_http_error is not None else {}),
        **({"diff_truncated": llm._last_review_truncated_bytes}
           if llm._last_review_truncated_bytes else {}),
        **v2_metrics,
    })
...
def _maybe_bootstrap_agent_sdk_async():
    ...
    Under FREECODE_CODE_SYNC_PLUGIN_INSTALL=true (CCR-style remote pods),
    ...
    try:
        ...
        if importlib.util.find_spec("freecode_agent_sdk") is not None:
            return
        import time as _t
        try:
...
def main():
    ...
    # bootstrap that remote pods miss under FREECODE_CODE_SYNC_PLUGIN_INSTALL.
    ...
    if tool_name in ["Edit", "Write", "MultiEdit", "NotebookEdit"]:
        ...
        # Skip plan files
        plans_dir = os.path.expanduser("~/.freecode/plans")
        ...
        if ENABLE_PATTERN_RULES:
            ...
            # This prevents flagging pre-existing insecure patterns when FreeCode rewrites a file
            if tool_name == "Write" and pattern_matches:
                cwd = os.environ.get("FREECODE_PROJECT_DIR", os.getcwd())
