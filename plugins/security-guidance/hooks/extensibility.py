...
1. ``freecode-security-guidance.md`` — markdown appended to every LLM review prompt.
...
Discovery, in precedence order (matching FREECODE.md / settings.json):
  - ``~/.freecode/<name>``                  (user)
  - ``<cwd>/.freecode/<name>``              (project, committed)
  - ``<cwd>/.freecode/<name>.local.<ext>``  (project local, gitignored)

Managed delivery via ``managed-settings.json`` is not yet supported.
Org admins can still push files to ``~/.freecode/`` via MDM/GPO.
...
GUIDANCE_BASENAME = "freecode-security-guidance.md"
...
def load_for_session(cwd: Optional[str]) -> None:
    ...
    try:
        ...
        debug_log(f"extensibility: failed to load freecode-security-guidance.md: {e}")
        _guidance_block = ""
    try:
...
def _config_paths(cwd: Optional[str], basename: str) -> List[Tuple[str, str]]:
    ...
    paths = [("User", os.path.expanduser(os.path.join("~", ".freecode", basename)))]
    if cwd:
        paths.append(("Project", os.path.join(cwd, ".freecode", basename)))
        # freecode-security-guidance.local.md / security-patterns.local.yaml
        stem, ext = os.path.splitext(basename)
        paths.append(("Project (local)", os.path.join(cwd, ".freecode", f"{stem}.local{ext}")))
    return paths
...
def _load_guidance(cwd: Optional[str]) -> str:
    ...
    if len(combined) > GUIDANCE_MAX_BYTES:
        debug_log(
            f"extensibility: freecode-security-guidance.md combined size "
...
def _load_user_patterns(cwd: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for label, path in _config_paths(cwd, "security-patterns"):
        ...
        # ".freecode/security-patterns" or ".freecode/security-patterns.local");
