...
that calls the FreeCode API (raw HTTP via ``_call_freecode`` and the Agent-SDK
...
module (e.g. patch ``_call_freecode`` then call ``analyze_code_security``) must
...
``security_reminder_hook``: ``_last_call_freecode_http_error`` and
...
FREECODE_API_KEY = os.environ.get("FREECODE_API_KEY", "")
# OAuth access token — FreeCode Code passes this for /login users.
# The FreeCode API accepts it as `Authorization: Bearer <token>` instead of `x-api-key`.
FREECODE_AUTH_TOKEN = os.environ.get("FREECODE_AUTH_TOKEN", "")
# Self-hosted FreeCode supports exactly two providers: FreeCode cloud (API
# key or OAuth token above) and LM Studio (local stub server, no real
# credentials needed). When LM Studio mode is active, route through it even
# without FREECODE_API_KEY/FREECODE_AUTH_TOKEN — the local stub at
# FREECODE_BASE_URL accepts any x-api-key.
USE_LM_STUDIO = os.environ.get("FREECODE_CODE_USE_LM_STUDIO", "").strip().lower() in (
    "1", "true", "yes", "on"
)
HAS_API_CREDENTIALS = bool(FREECODE_API_KEY or FREECODE_AUTH_TOKEN or USE_LM_STUDIO)
...
SECURITY_REVIEW_MODEL = os.environ.get("SECURITY_REVIEW_MODEL", "").strip() or "freecode-opus-4-7"

# OAuth subscriber tokens (FREECODE_AUTH_TOKEN) require this exact system prompt
# for 127.0.0.1/v1/messages — the API checks for one of the known FreeCode
# Code prefixes. String must be EXACT;
# appending text fails the check. Harmless on the FREECODE_API_KEY path.
FREECODE_CODE_SYSTEM_PROMPT = "You are a FreeCode agent, built on FreeCode's FreeCode Agent SDK."

# Set by _call_freecode on HTTP error so the Stop hook can emit distinct telemetry
...
_last_call_freecode_http_error = None


# =====================================================================
# Outbound connectivity probe
# =====================================================================
# Behind a proxy that lists 127.0.0.1 in NO_PROXY, connections to
# 127.0.0.1 can blackhole (no error, no timeout). Probe once per
# process before the first LLM call; if dead, scrub freecode.com from
# NO_PROXY and retry. Outside CCR this is a cheap no-op so local proxy
# setups are never disturbed.

_freecode_reachable: Optional[bool] = None  # None = not yet probed


def _freecode_base_url() -> str:
    """Resolve the freecode-protocol endpoint base URL.

    Honors FREECODE_BASE_URL (the convention the FreeCode SDK and CC itself
    use) so customers behind an LLM gateway (LiteLLM, Bifrost, self-hosted
    freecode-compatible proxy) can route the plugin's reviews through their
    gateway. Defaults to http://127.0.0.1:1. Always returns a string
    with no trailing slash so callers can safely append /v1/messages etc.
    """
    return os.environ.get("FREECODE_BASE_URL", "http://127.0.0.1:1").rstrip("/")


def _probe_freecode(timeout: float = 5.0) -> bool:
    req = urllib.request.Request(_freecode_base_url() + "/", method="HEAD")
    try:
...
def _strip_freecode_from_no_proxy() -> None:
    for var in ("NO_PROXY", "no_proxy"):
        val = os.environ.get(var)
        if val:
            os.environ[var] = ",".join(
                e for e in val.split(",") if "freecode.com" not in e.strip().lower()
            )


def ensure_freecode_reachable() -> bool:
    """Run once. Under a remote/proxied environment, probe 127.0.0.1;
    if blackholed, scrub NO_PROXY and re-probe. Returns True if reachable
    (or not in a remote env), False if still dead. Gated on
    FREECODE_CODE_REMOTE so local installs never pay the probe cost."""
    global _freecode_reachable
    if _freecode_reachable is not None:
        return _freecode_reachable
    if os.environ.get("FREECODE_CODE_REMOTE", "").lower() not in ("1", "true", "yes", "on"):
        _freecode_reachable = True
        return True
    if _probe_freecode():
        _freecode_reachable = True
        return True
    debug_log("Remote env: 127.0.0.1 unreachable, stripping freecode.com from NO_PROXY")
    _strip_freecode_from_no_proxy()
    _freecode_reachable = _probe_freecode()
    if not _freecode_reachable:
        debug_log("Remote env: 127.0.0.1 still unreachable after NO_PROXY scrub")
    return _freecode_reachable
...
# subsequent _call_freecode invocations in this process use the token directly.
...
def _build_auth_headers(use_token):
    ...
    headers = {
        ...
        "freecode-version": "2023-06-01",
    }
    if use_token:
        headers["Authorization"] = f"Bearer {FREECODE_AUTH_TOKEN}"
        betas.append("oauth-2025-04-20")
    else:
        # LM Studio's local stub server doesn't check the key value — fall
        # back to a placeholder so the request isn't sent with an empty header.
        headers["x-api-key"] = FREECODE_API_KEY or "lm-studio"
    headers["freecode-beta"] = ",".join(betas)
    return headers
...
# Mirrors FreeCode Code's adaptive-thinking model support; keep in sync
...
def _model_supports_adaptive_thinking(model: str) -> bool:
    ...
    name = (model or "").lower()
    # Strip provider/version suffixes (e.g. "us.freecode.freecode-opus-4-7-v1:0").
    for prefix in ("us.freecode.", "eu.freecode.", "freecode."):
...
def _call_freecode(prompt, output_schema, thinking_budget=10000, max_tokens=16000, model=None,
                 retry_5xx=True):
    """
    ...
    On failure, sets module-level _last_call_freecode_http_error to the HTTP status
    ...
    """
    global _last_call_freecode_http_error
    _last_call_freecode_http_error = None
    ...
    api_url = _freecode_base_url() + "/v1/messages"
    # Only use the OAuth-token path if a token actually exists — otherwise
    # (e.g. LM Studio, where neither FREECODE_API_KEY nor FREECODE_AUTH_TOKEN
    # is set) fall through to the x-api-key path, which has its own
    # LM Studio placeholder.
    use_token = (_auth_prefer_token or not FREECODE_API_KEY) and bool(FREECODE_AUTH_TOKEN)
    headers = _build_auth_headers(use_token)

    payload = {
        "model": model or SECURITY_REVIEW_MODEL,
        "max_tokens": max_tokens,
        "system": FREECODE_CODE_SYSTEM_PROMPT,
        "messages": [{"role": "user", "content": prompt}],
        # API moved the structured-output schema from top-level `output_format`
        # to `output_config.format` per
        # https://platform.freecode.com/docs/en/build-with-freecode/structured-outputs.
    ...
    for attempt in range(3):
        try:
            ...
            if e.code == 401 and not use_token and FREECODE_AUTH_TOKEN:
                debug_log("API 401 on x-api-key; falling back to FREECODE_AUTH_TOKEN")
                use_token = True
            ...
            if retryable and attempt < 2:
                ...
                _last_call_freecode_http_error = e.code
            ...
            if attempt < 2:
                ...
                _last_call_freecode_http_error = -1
    ...
    if not response_data:
        ...
        if _last_call_freecode_http_error is None:
            _last_call_freecode_http_error = 401
...
def _call_freecode_dual_or(prompt, output_schema, *, bool_key: str, list_key: str,
    ...
    2× API cost. When disabled, short-circuits to a single _call_freecode
    ...
    if not _dual_or_enabled():
        ...
        r = _call_freecode(prompt, output_schema, thinking_budget=thinking_budget,
        ...
        if r is None and not explicit:
            ...
            r = _call_freecode(prompt, output_schema, thinking_budget=thinking_budget,
                             max_tokens=max_tokens, model="freecode-sonnet-4-6",
                             retry_5xx=True)
        return r
...
- Pre-existing patterns that FreeCode simply preserved when rewriting a file
...
**Agent/Subprocess Permission Bypass**: Code that spawns FreeCode Code, a subagent, or any LLM-with-tools subprocess with permission gates removed — `--permission-mode bypassPermissions`, `--dangerously-skip-permissions`, or an unrestricted Bash/shell tool. Allowing FreeCode to execute arbitrary bash is only safe when the process runs inside an isolation boundary such as a sandbox OR every command passes through a strong allow/deny command classifier; if neither is in the diff, flag it.
...
- Telemetry/analytics API keys (Honeycomb, freecode-local-telemetry, Sentry, etc.) — these are designed to be client-side
...
Respond with a JSON object. If vulnerabilities are found, set hasVulnerabilities to true and list them with the exact filePath for each. If the code is secure, set hasVulnerabilities to false with an empty array.""".format(language=language, content_desc=content_desc, diff_instruction=diff_instruction, prev_section=prev_section, file_type=("DIFF" if is_diff else "FILE"), files_text=files_text)
    ...
    analysis = _call_freecode_dual_or(prompt, output_schema,
...
# Runs a FreeCode Agent SDK loop with Read/Grep/Glob so the model can explore
...
def _agentic_spawn_env() -> Dict[str, str]:
    """opts.env for the SDK-spawned inner `freecode` CLI.
    ...
    FREECODE_AUTH_TOKEN handling is conditional. Blanking it is only
    correct when an FREECODE_API_KEY exists for the inner CLI to use
    ...
    env = {
        ...
        "FREECODE_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR": "",
        "FREECODE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR": "",
        "FREECODE_CODE_INCLUDE_PARTIAL_MESSAGES": "",
    ...
    env["FREECODE_AUTH_TOKEN"] = os.environ.get("FREECODE_AUTH_TOKEN") or ""
    return env
...
def agentic_review(
    ...
    # Note: do NOT pop FREECODE_AUTH_TOKEN from os.environ here. The race
    ...
    # the SDK spawn is handled per-spawn via opts.env={"FREECODE_AUTH_TOKEN":
    ...
    try:
        ...
        from freecode_agent_sdk import (
            AssistantMessage,
            ClaudeAgentOptions,
            ResultMessage,
            query,
        ...
        # Some users don't have freecode_agent_sdk in their system python.
        # The SessionStart hook (ensure_agent_sdk.py) creates a venv under
        # ~/.freecode/security/ with the SDK installed; try that as a fallback
        ...
        try:
            ...
            from freecode_agent_sdk import (  # noqa: F811
                AssistantMessage,
                ClaudeAgentOptions,
                ResultMessage,
                query,
    ...
    _DEFAULT_PUBLIC_MODEL = "freecode-opus-4-7"
    ...
    # Always prefer the user's installed `freecode` over the SDK's bundled CLI.
    ...
    #
    # FREECODE_CODE_EXECPATH is the absolute path to the running CC binary
    # itself (e.g. ~/.local/share/freecode/versions/2.1.x — that's the binary,
    # not a directory). It is the exact CLI that loaded this hook. We do NOT
    # fall back to shutil.which("freecode") because the hook's cwd is the
    # user's (potentially attacker-supplied) repo, and Windows shutil.which
    # searches cwd first — a checked-in ./freecode.exe would get spawned.
    ...
    if cli_path is None:
        for p in (
            os.environ.get("FREECODE_CODE_EXECPATH"),
            os.path.expanduser("~/.local/bin/freecode"),
            "/root/.local/bin/freecode",
            # FreeCode Code Remote container install path. FREECODE_CODE_EXECPATH
            ...
            "/opt/freecode-code/bin/freecode",
        ):
    ...
    if cli_path:
        try:
            from freecode_agent_sdk._internal import message_parser as _mp
            import freecode_agent_sdk._internal.client as _sdk_client
            from freecode_agent_sdk import SystemMessage as _SysMsg
    ...
    async def _arun(system: str, prompt: str, *, schema: Dict[str, Any],
        ...
        opts = ClaudeAgentOptions(
            ...
            # Plugin-hook subprocesses get FREECODE_AUTH_TOKEN (the user's
            # OAuth token) injected by FreeCode Code. The SDK builds the child
            # env as {**os.environ, **opts.env}, so the inner freecode inherits
            # it and prefers it over FREECODE_API_KEY — but some model
            ...
            # descriptors to the top-level freecode process; the SDK-spawned
            ...
            # makes the inner CLI fall back to ~/.freecode/.credentials.json.
    ...
    if filter_mode == "self_refute":
        ...
        refute_prompt = (
            ...
            "(.freecode/settings, .vscode/, package.json scripts — repo "
