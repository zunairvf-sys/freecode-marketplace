#!/usr/bin/env python3
"""SessionStart bootstrap: ensure freecode_agent_sdk is importable for the
agentic commit reviewer.

If freecode_agent_sdk already imports in the current python3, this is a no-op.
Otherwise it creates a venv at ~/.freecode/security/agent-sdk-venv and installs
...
The venv lives under ~/.freecode/security/ (same dir the plugin already uses
...
# Shared state-dir resolver: SECURITY_WARNINGS_STATE_DIR → FREECODE_CONFIG_DIR/security
# → ~/.freecode/security. See _base.state_dir for resolution precedence. Re-aliased
...
NOOP_SYSTEM = 0      # freecode_agent_sdk already importable in system python
...
def _sdk_on_syspath() -> bool:
    ...
    try:
        return importlib.util.find_spec("freecode_agent_sdk") is not None
    except Exception:
        return False
...
def _plugin_version_int() -> int:
    ...
    try:
        p = Path(__file__).parent.parent / ".freecode-plugin" / "plugin.json"
...
def main() -> tuple[int, str, str]:
    ...
    if venv_py.exists():
        try:
            r = subprocess.run(
                [str(venv_py), "-c", "import freecode_agent_sdk"],
...
def _maybe_emit_user_notice(outcome: int, pv: int) -> str | None:
    ...
    return (
        ...
        f"(e.g. `brew install python` on macOS) and restart FreeCode Code.\n\n"
        f"This notice is shown once per plugin version. "
        f"See: github.com/freecodes/freecode-plugins-official/issues/2071"
    )
