...
# New session call
Bash({
  command: "~/.freecode/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.freecode/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend gemini --gemini-model gemini-3-pro-preview resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
| Phase | Gemini |
|-------|--------|
| Analysis | `~/.freecode/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.freecode/.ccg/prompts/gemini/architect.md` |
| Review | `~/.freecode/.ccg/prompts/gemini/reviewer.md` |
...
freecode synthesizes plan, save to `.freecode/plan/task-name.md` after user approval.
...
4. freecode handles all code writes and file operations
