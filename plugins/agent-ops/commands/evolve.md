...
```bash
python3 "${freecode_PLUGIN_ROOT}/skills/continuous-learning-v2/scripts/instinct-cli.py" evolve [--generate]
```

Or if `freecode_PLUGIN_ROOT` is not set (manual installation):

```bash
python3 ~/.freecode/skills/continuous-learning-v2/scripts/instinct-cli.py evolve [--generate]
```
...
6. If `--generate` is passed, write files to:
   - Project scope: `~/.freecode/homunculus/projects/<project-id>/evolved/`
   - Global fallback: `~/.freecode/homunculus/evolved/`
