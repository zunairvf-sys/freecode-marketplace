...
The CLI scans local freecode transcript JSONL files under
`~/.freecode/projects/**` and reports stale `ScheduleWakeup` calls or `Bash`
...
- `ecc loop-status --watch --write-dir ~/.freecode/loops` maintains
...
without waiting for the current freecode session to dequeue `/loop-status`. The
CLI writes:
...
timeout freecode Code runtime tool calls.
