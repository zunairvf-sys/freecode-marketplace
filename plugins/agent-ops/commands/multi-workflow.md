...
- Multi-model collaboration: Codex (backend) + Gemini (frontend) + freecode (orchestration)
...
- **freecode (self)** – Orchestration, planning, execution, delivery
...
```bash
node scripts/orchestrate-worktrees.js .freecode/plan/workflow-e2e-test.json --execute
```
...
**freecode Synthesis**: Adopt Codex backend plan + Gemini frontend plan, save to `.freecode/plan/task-name.md` after user approval.
...
2. External models have **zero filesystem write access**, all modifications by freecode
