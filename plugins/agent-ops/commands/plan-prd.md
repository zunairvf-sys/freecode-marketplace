...
| Write `.freecode/prds/{name}.prd.md` | Produce an implementation plan — that's `/plan` |
...
```bash
mkdir -p .freecode/prds
```

**Output path**: `.freecode/prds/{kebab-case-name}.prd.md`
...
```
PRD created: .freecode/prds/{name}.prd.md
...
Next step: /plan .freecode/prds/{name}.prd.md
