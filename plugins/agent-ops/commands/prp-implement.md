...
```bash
mkdir -p .freecode/PRPs/reports
```

Write report to `.freecode/PRPs/reports/{plan-name}-report.md`:
...
```bash
mkdir -p .freecode/PRPs/plans/completed
mv "$ARGUMENTS" .freecode/PRPs/plans/completed/
```
...
### Artifacts
- Report: `.freecode/PRPs/reports/{name}-report.md`
- Archived Plan: `.freecode/PRPs/plans/completed/{name}.plan.md`
