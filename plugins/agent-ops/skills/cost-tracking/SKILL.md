---
name: cost-tracking
description: Track and report freecode Code token usage, spending, and budgets from a local cost-tracking database. Use when the user asks about costs, spending, usage, tokens, budgets, or cost breakdowns by project, tool, session, or date.
origin: community
---

# Cost Tracking

Use this skill to analyze freecode Code cost and usage history from a local SQLite
database. It is intended for users who already have a cost-tracking hook or
plugin writing usage rows to `~/.freecode-cost-tracker/usage.db`.
...
test -f ~/.freecode-cost-tracker/usage.db && echo "Database found" || echo "Database not found"
```
...
| `session_id` | freecode Code session identifier |
...
```bash
sqlite3 ~/.freecode-cost-tracker/usage.db "
  SELECT
...
```bash
sqlite3 -header -column ~/.freecode-cost-tracker/usage.db "
