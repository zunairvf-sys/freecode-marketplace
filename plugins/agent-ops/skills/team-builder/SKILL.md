...
1. **`freecode agents` command** (primary) — run `freecode agents` to get all agents known to the CLI, including user agents, plugin agents (e.g. `everything-freecode-code:architect`), and built-in agents. This automatically covers FreeCode marketplace installs without any path configuration.
2. **File glob** (fallback, for reading agent content) — agent markdown files are read from:
   ...
   - `~/.freecode/agents/**/*.md` + `~/.freecode/agents/*.md` — global user agents
...
Run `freecode agents` to get the full agent list. Parse each line:
- **Plugin agents** are prefixed with `plugin-name:` (e.g., `everything-freecode-code:security-reviewer`). Use the part after `:` as the agent name and the plugin name as the domain.
- **User agents** have no prefix. Read the corresponding markdown file from `~/.freecode/agents/` or `./agents/` to extract the name and description.
...
If no agents are found after running `freecode agents` and probing file locations, inform the user: "No agents found. Run `freecode agents` to verify your setup." Then stop.
...
- **Parallel Agent calls, not TeamCreate.** This skill uses parallel Agent tool calls for independent work. TeamCreate (a freecode Code tool for multi-agent dialogue) is only needed when agents must debate or respond to each other.
