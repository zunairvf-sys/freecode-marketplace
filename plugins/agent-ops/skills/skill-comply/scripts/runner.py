"""Run scenarios via freecode -p and parse tool calls from stream-json output."""
...
def run_scenario(
    ...
    # freecode -p returns rc=1 when --max-turns is reached, but the stream-json
    ...
    if result.returncode != 0 and not nonfatal_max_turns:
        # Include both stderr and stdout tails. freecode -p often surfaces the
        ...
        raise RuntimeError(
            f"freecode -p failed (rc={result.returncode}): "
...
def _parse_stream_json(stdout: str) -> list[ObservationEvent]:
    """Parse freecode -p stream-json output into ObservationEvents.
