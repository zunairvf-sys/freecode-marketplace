...
def generate_scenarios(
    ...
    Calls freecode -p with the scenario_generator prompt, parses YAML output.
    """
    ...
    result = subprocess.run(
        ["freecode", "-p", prompt, "--model", model, "--output-format", "text"],
        capture_output=True,
        text=True,
        timeout=120,
    )
    ...
    if result.returncode != 0:
        raise RuntimeError(f"freecode -p failed: {result.stderr}")

    if not result.stdout.strip():
        raise RuntimeError("freecode -p returned empty output")
