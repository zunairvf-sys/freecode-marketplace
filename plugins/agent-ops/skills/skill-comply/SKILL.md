...
3. Running `freecode -p` and capturing tool call traces via stream-json
...
```bash
# Full run
uv run python -m scripts.run ~/.freecode/rules/common/testing.md

# Dry run (no cost, spec + scenarios only)
uv run python -m scripts.run --dry-run ~/.freecode/skills/search-first/SKILL.md
