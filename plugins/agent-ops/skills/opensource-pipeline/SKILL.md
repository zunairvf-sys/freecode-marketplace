...
Safely open-source any project through a 3-stage pipeline: **Fork** (strip secrets) → **Sanitize** (verify clean) → **Package** (freecode.md + setup.sh + README).
...
| `/opensource package PROJECT` | Generate freecode.md + setup.sh + README |
...
Generate:
1. freecode.md (commands, architecture, key files)
...
Show each project with pipeline progress (FORK_REPORT.md, SANITIZATION_REPORT.md, freecode.md presence).
...
$HOME/opensource-staging/
  my-project/
    ...
    freecode.md                # From packager agent
