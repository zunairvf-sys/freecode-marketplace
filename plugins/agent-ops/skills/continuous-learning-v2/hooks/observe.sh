...
# freecode Code passes hook data via stdin as JSON.
#
...
# Can also be registered manually in ~/.freecode/settings.json.

set -e

# Hook phase from CLI argument: "pre" (PreToolUse) or "post" (PostToolUse).
# Manual settings.json installs can call this script without the plugin
# wrapper's positional phase argument, but freecode Code still exposes the hook
# event name in freecode_HOOK_EVENT_NAME.  Fall back to that env var before
...
if [ -z "$HOOK_PHASE" ]; then
  case "${freecode_HOOK_EVENT_NAME:-}" in
...
# Read JSON from stdin (freecode Code hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  if [ -n "$_GIT_ROOT" ]; then
    export freecode_PROJECT_DIR="$_GIT_ROOT"
    unset CLV2_NO_PROJECT
  else
    unset freecode_PROJECT_DIR
...
#   - FreeCode observing its own Haiku observer sessions (self-loop)
#   - FreeCode observing other tools' automated sessions
...
case "${freecode_CODE_ENTRYPOINT:-cli}" in
  cli|sdk-ts|freecode-desktop) ;;
  *) exit 0 ;;
esac
...
_ECC_SKIP_PATHS="${ECC_OBSERVE_SKIP_PATHS:-observer-sessions,.freecode-mem}"
if [ -n "$STDIN_CWD" ]; then
...
# Pass HOOK_PHASE via env var since freecode Code does not include hook type in stdin JSON
...
try:
    ...
    # freecode Code does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - freecode Code hook format
...
# which caused runaway parallel freecode analysis processes.
