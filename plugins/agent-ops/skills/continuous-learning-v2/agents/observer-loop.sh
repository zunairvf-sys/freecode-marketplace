...
# parallel freecode analysis processes.
...
wait_for_freecode_analysis() {
  ...
  while true; do
    ...
    # SIGUSR1 can interrupt wait while the freecode child is still running.
...
analyze_observations() {
  ...
  if [ "${CLV2_IS_WINDOWS:-false}" = "true" ] && [ "${ECC_OBSERVER_ALLOW_WINDOWS:-false}" != "true" ]; then
    echo "[$(date)] Skipping freecode analysis on Windows due to known non-interactive hang issue (#295). Set ECC_OBSERVER_ALLOW_WINDOWS=true to override." >> "$LOG_FILE"
    return
  fi

  if ! command -v freecode >/dev/null 2>&1; then
    echo "[$(date)] freecode CLI not found, skipping analysis" >> "$LOG_FILE"
    return
  fi
  ...
  # prefixes (e.g. /c/Users/...) that the freecode subprocess cannot resolve.
...
PROMPT
  ...
  # Read the prompt into memory before the freecode subprocess is spawned.
  # On Windows/MSYS2, the mktemp path can differ from the shell's later path
  # resolution, so relying on cat "$prompt_file" inside the freecode invocation
  ...
  ECC_SKIP_OBSERVE=1 ECC_HOOK_PROFILE=minimal freecode --model haiku --max-turns "$max_turns" --print \
  ...
  (
    ...
    if kill -0 "$freecode_pid" 2>/dev/null; then
      echo "[$(date)] freecode analysis timed out after ${timeout_seconds}s; terminating process" >> "$LOG_FILE"
      kill "$freecode_pid" 2>/dev/null || true
    fi
  ) &
  watchdog_pid=$!

  wait_for_freecode_analysis "$freecode_pid"
  exit_code=$?
  kill "$watchdog_pid" 2>/dev/null || true
  rm -f "$analysis_file"

  if [ "$exit_code" -ne 0 ]; then
    echo "[$(date)] freecode analysis failed (exit $exit_code)" >> "$LOG_FILE"
  fi
