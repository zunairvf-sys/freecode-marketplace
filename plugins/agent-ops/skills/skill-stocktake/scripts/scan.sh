...
#
# When CWD_SKILLS_DIR is omitted, defaults to $PWD/.freecode/skills so the
# script always picks up project-level skills without relying on the caller.
#
# Environment:
#   SKILL_STOCKTAKE_GLOBAL_DIR   Override ~/.freecode/skills (for testing only;
#                                do not set in production — intended for bats tests)
#   SKILL_STOCKTAKE_PROJECT_DIR  Override project dir detection (for testing only)

set -euo pipefail

GLOBAL_DIR="${SKILL_STOCKTAKE_GLOBAL_DIR:-$HOME/.freecode/skills}"
CWD_SKILLS_DIR="${SKILL_STOCKTAKE_PROJECT_DIR:-${1:-$PWD/.freecode/skills}}"
...
OBSERVATIONS="${SKILL_STOCKTAKE_OBSERVATIONS:-$HOME/.freecode/observations.jsonl}"

# Validate CWD_SKILLS_DIR looks like a .freecode/skills path (defense-in-depth).
# Only warn when the path exists — a nonexistent path poses no traversal risk.
if [[ -n "$CWD_SKILLS_DIR" && -d "$CWD_SKILLS_DIR" && "$CWD_SKILLS_DIR" != */.freecode/skills* ]]; then
  echo "Warning: CWD_SKILLS_DIR does not look like a .freecode/skills path: $CWD_SKILLS_DIR" >&2
fi
