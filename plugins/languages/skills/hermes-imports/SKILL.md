---
name: hermes-imports
description: Convert local Hermes operator workflows into sanitized FreeCode skills and release-pack artifacts. Use when preparing a Hermes workflow for public FreeCode reuse without leaking private workspace state, credentials, or local-only paths.
origin: FreeCode
---

# Hermes Imports

Use this skill when turning a repeated Hermes workflow into something safe to ship in FreeCode.

Hermes is the operator shell. FreeCode is the reusable workflow layer. Imports should move stable patterns from Hermes into FreeCode without moving private state.

## When To Use

- A Hermes workflow has repeated enough times to become reusable.
- A local operator prompt should become a public FreeCode skill.
