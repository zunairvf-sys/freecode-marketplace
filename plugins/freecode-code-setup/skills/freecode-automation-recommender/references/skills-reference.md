...
Skills are packaged expertise with workflows, reference materials, and best practices. Create them in `.freecode/skills/<name>/SKILL.md`. Skills can be invoked by FreeCode automatically when relevant, or by users directly with `/skill-name`.
...
Create project-specific skills in `.freecode/skills/<name>/SKILL.md`.
...
user-invocable: false           # Only FreeCode can invoke (for background knowledge)
...
```
.freecode/skills/create-migration/
├── SKILL.md
└── scripts/
...
3. Run validation: `bash ~/.freecode/skills/create-migration/scripts/validate-migration.sh`
...
### Project Conventions (freecode-only)

Background knowledge FreeCode applies automatically:
...
The command output replaces the placeholder before FreeCode sees the skill content.
