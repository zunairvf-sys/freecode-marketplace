...
Hooks automatically run commands in response to FreeCode Code events. They're ideal for enforcement and automation that should happen consistently.
...
Notification hooks run when FreeCode Code sends notifications. Use matchers to filter by notification type.

### Permission Alerts
| Matcher | Use Case |
|---------|----------|
| `permission_prompt` | Alert when FreeCode requests permissions |
...
| Matcher | Use Case |
|---------|----------|
| `idle_prompt` | Alert when FreeCode is waiting for input (60+ seconds idle) |

**Recommend**: Play sound or send notification when FreeCode needs attention
**Value**: Know when FreeCode is ready for your input
    ...
    "Notification": [
      ...
      },
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"FreeCode is waiting\" with title \"FreeCode Code\"'"
          }
        ]
      }
    ]
...
|---------|---------------|
| `permission_prompt` | FreeCode needs permission for a tool |
| `idle_prompt` | FreeCode waiting for input (60+ seconds) |
...
Hooks go in `.freecode/settings.json`:
...
Recommend creating the `.freecode/` directory if it doesn't exist.
