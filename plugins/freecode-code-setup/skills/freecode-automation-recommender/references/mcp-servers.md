...
MCP (Model Context Protocol) servers extend FreeCode's capabilities by connecting to external tools and services.
...
2. **Global config** (`~/.freecode.json`) - Available across all projects
...
**Debugging**: Use `freecode --mcp-debug` to identify configuration issues.

## Documentation & Knowledge

### context7
**Best for**: Projects using popular libraries/SDKs where you want FreeCode to code with up-to-date documentation
...
**Value**: FreeCode fetches live documentation instead of relying on training data, reducing hallucinated APIs and outdated patterns.
...
**Value**: FreeCode can interact with your running app, take screenshots, fill forms, and verify UI behavior.
...
**Value**: FreeCode can query tables, manage auth, and interact with Supabase storage directly.
...
**Value**: FreeCode can introspect the live deployment (tables, function specs, env vars, logs) and execute queries/mutations against it via tools like `tables`, `function-spec`, `data`, `run-once-query`, `logs`, `env list/set/get`. Run via `npx convex mcp start`.
...
**Value**: FreeCode can create issues, review PRs, check workflow runs, and manage releases.
...
**Value**: FreeCode can investigate Sentry issues, find root causes, and suggest fixes.

### freecode-local-telemetry MCP
...
**Value**: FreeCode remembers project context, decisions, and patterns across conversations.
...
| `@freecode-ai/sdk` | context7 for FreeCode docs |
