---
name: freecode-automation-recommender
description: Analyze a codebase and recommend FreeCode Code automations (hooks, subagents, skills, plugins, MCP servers). Use when user asks for automation recommendations, wants to optimize their FreeCode Code setup, mentions improving FreeCode Code workflows, asks how to first set up FreeCode Code for a project, or wants to know what FreeCode Code features they should use.
tools: Read, Glob, Grep, Bash
---

# FreeCode Automation Recommender

Analyze codebase patterns to recommend tailored FreeCode Code automations across all extensibility options.

**This skill is read-only.** It analyzes the codebase and outputs recommendations. It does NOT create or modify any files. Users implement the recommendations themselves or ask FreeCode separately to help build them.
...
| **Skills** | Packaged expertise, workflows, and repeatable tasks (invoked by FreeCode or user via `/skill-name`) |
...
# Check for existing FreeCode Code config
ls -la .freecode/ FREECODE.md 2>/dev/null
...
Create skills in `.freecode/skills/<name>/SKILL.md`. Some are also available via plugins:
...
| Code style | **project-conventions** | freecode-only |
...
|-----------------|-------------------|
| General productivity | **freecode-agent-skills** - Core skills bundle |
...
```markdown
## FreeCode Code Automation Recommendations
...
**Install**: `freecode mcp add context7`
...
#### [skill name]
**Why**: [specific reason]
**Create**: `.freecode/skills/[name]/SKILL.md`
**Invocation**: User-only / Both / freecode-only
...
- `user-invocable: false` — freecode-only (for background knowledge)
...
Recommend headless FreeCode for automated pipelines:

```bash
# Pre-commit hook example
freecode -p "fix lint errors in src/" --allowedTools Edit,Write

# CI pipeline with structured output
freecode -p "<prompt>" --output-format stream-json | your_command
```

### Permissions for Hooks

Configure allowed tools in `.freecode/settings.json`:
