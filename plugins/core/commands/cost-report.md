---
description: Generate a local freecode Code cost report from a cost-tracker SQLite database.
argument-hint: [csv]
---
...
is already writing usage rows to `~/.freecode-cost-tracker/usage.db`.

## What This Command Does

1. Check that `sqlite3` is available.
2. Check that `~/.freecode-cost-tracker/usage.db` exists.
...
trusted freecode Code cost-tracking hook/plugin first.
...
```bash
sqlite3 -csv -header ~/.freecode-cost-tracker/usage.db "
