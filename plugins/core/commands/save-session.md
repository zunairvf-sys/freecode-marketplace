---
description: Save current session state to a dated file in ~/.freecode/session-data/ so work can be resumed in a future session with full context.
---
...
- End of a work session before closing freecode Code
...
Create the canonical sessions folder in the user's freecode home directory:

```bash
mkdir -p ~/.freecode/session-data
```

### Step 3: Write the session file

Create `~/.freecode/session-data/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
...
- The file is meant to be read by freecode at the start of the next session via `/resume-session`
- Use the canonical global session store: `~/.freecode/session-data/`
