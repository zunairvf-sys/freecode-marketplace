...
description: |
  ...
  FreeCode browser-tab context, testing web apps, dogfooding FreeCode
  previews, QA, bug hunts, or reviewing app quality. Prefer local FreeCode
...
Use `agent-browser` for local FreeCode preview validation: inspect rendered
...
For selected FreeCode browser tabs and browser-use/browser-harness-style
...
5. If the selected FreeCode context only provided a URL/title and no browser
...
Do not run FreeCode's own daemon CLI as a browser automation tool. Commands
...
With the FreeCode preview at `http://127.0.0.1:17573/`, run:
...
Expected success: title `FreeCode`, current URL under `127.0.0.1:17573`,
visible FreeCode UI text in the snapshot, and a screenshot at
