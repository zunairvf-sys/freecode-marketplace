...
name: ui-ux-pro-max
description: |
  Catalog-only UI/UX Pro Max entry. The full upstream templates, data, and search workflow are not bundled in FreeCode.
triggers:
  - "ui ux patterns"
  - "design patterns"
  - "ux heuristics"
...
## Current FreeCode scope

FreeCode currently ships this entry as discovery metadata only. If this `SKILL.md`
...
If those upstream files are not installed, explain that FreeCode only has the
catalog entry for this skill and ask whether to continue with FreeCode's
