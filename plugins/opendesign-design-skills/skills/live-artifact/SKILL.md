...
name: live-artifact
description: |
  Create refreshable, auditable FreeCode artifacts backed by connector or local data.
...
Create an FreeCode live artifact: a project-scoped, previewable HTML artifact whose data can later be refreshed without redesigning the presentation.
...
5. **Register or update through daemon wrappers**
   - Use the FreeCode daemon wrapper commands via `"$OD_NODE_BIN" "$OD_BIN"` instead of raw `curl`, bare `node`, or bare `od`:
